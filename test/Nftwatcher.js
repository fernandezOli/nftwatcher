...
describe("Nftwatcher: Tests", function () {
  ...
  beforeEach(async function () {
    ...
    await Nftwatcher.deployed();
  });

  describe("Tests unitaires", function () {
    it("Should return the good value", async function () {
      expect(await Nftwatcher.name()).to.equal("Nftwatcher");
      expect(await Nftwatcher.symbol()).to.equal("NFTW");
    });
  });

  describe("Tests buy", function () {
    it("Buy NFT", async function () {
      await expect(Nftwatcher.buy()).to.be.reverted;
      await Nftwatcher.buy({value: BigInt(10**17)});
      expect(await Nftwatcher.balanceOf(owner.address)).to.equal(1);
    });
  });
